    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
             ->addColumn('action', function($query){
                $deleteBtn = "<a href='".route('admin.flash-sale.destroy', $query->id)."' class='btn btn-danger ml-2 delete-item'><i class='fas fa-trash'></i>Delete</a>";
                return $deleteBtn;
            })
            ...
            ->addColumn('show_at_home', function($query){
                if($query->show_at_home == 1){
                    $button = '<label class="custom-switch mt-2">
                <input type="checkbox" checked class="custom-switch-input change-at-home-status" data-id="'.$query->id.'" name="custom-switch-checkbox" />
                <span class="custom-switch-indicator"></span>
                </label>';
                }else{
                    $button = '<label class="custom-switch mt-2">
                <input type="checkbox" class="custom-switch-input change-at-home-status" data-id="'.$query->id.'" name="custom-switch-checkbox" />
                <span class="custom-switch-indicator"></span>
                </label>';
                }
                return $button;
